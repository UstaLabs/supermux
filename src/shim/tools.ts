...
const ORCHESTRATION_TOOLS = [
  ...
  { name: "rename_session", description: "Rename your OWN session (the one you are running in). Use a short, descriptive name (lowercase, words joined by '-') when the session is new or its focus has changed. Returns the final name, which may be normalized or suffixed to stay unique.", inputSchema: { type: "object", properties: { name: { type: "string" } }, required: ["name"] } },
  ...
  { name: "expose_port", description: "Expose a local port to the web via reverse proxy. Returns the public URL. If domain is omitted, a random slug is generated. Set public=true to skip device-pairing auth (anyone with the URL can access).", inputSchema: { type: "object", properties: { port: { type: "number", description: "Local port to expose (1-65535)" }, domain: { type: "string", description: "Optional subdomain name (alphanumeric + hyphens, max 63 chars)" }, public: { type: "boolean", description: "When true, the proxy is reachable without pairing (default false)" } }, required: ["port"] } },
