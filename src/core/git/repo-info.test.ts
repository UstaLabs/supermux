...
test("getRepoInfo: non-git path is not eligible", async () => {
  const dir = mkdtempSync(join(tmpdir(), "mux-plain-"))
  expect(await getRepoInfo(dir)).toMatchObject({ isGitRepo: false, eligible: false })
})

test("getRepoInfo is async (never blocks the broker)", () => {
  const dir = mkdtempSync(join(tmpdir(), "mux-plain-"))
  expect(getRepoInfo(dir)).toBeInstanceOf(Promise)
})

test("getRepoInfo: a repo root is eligible and lists its branch", async () => {
  const dir = tmpRepo()
...
test("getRepoInfo: fetch:true is best-effort — a no-remote repo still lists branches", async () => {
  const dir = tmpRepo() // no remote configured → git fetch fails, but must be swallowed
  const info = await getRepoInfo(dir, { fetch: true })
  expect(info.isGitRepo).toBe(true)
  expect(info.branches?.local).toContain("main")
})

test("getRepoInfo: a parent dir containing a sub-repo is not a repo root", async () => {
  const parent = mkdtempSync(join(tmpdir(), "mux-parent-"))
  mkdirSync(join(parent, "sub"))
  execFileSync("git", ["init", "-b", "main", join(parent, "sub")])
  const info = await getRepoInfo(parent)
  expect(info.eligible).toBe(false)
})

test("getRepoInfo: remote list drops origin/HEAD symref (bare origin and …/HEAD)", async () => {
  const dir = tmpRepo()
