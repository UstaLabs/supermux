...
export type ClaudeSpawnOptions = { name: string; sessionId?: string; model?: string; effort?: string; role?: AgentRole; sessionRole?: SessionRole; claudeSessionId?: string; resume?: boolean; pluginsFile?: string; pluginsDir?: string; workdir?: string; rpcMcpConfig?: string }
export type ClaudeSpawnSpec = { argv: string[]; env: Record<string, string> }

// Platform-neutral source of truth for every Claude process decision. Runtime
// backends receive these argv/env boundaries directly; only the POSIX adapter
...
export function buildClaudeSpawnSpec(opts: ClaudeSpawnOptions): ClaudeSpawnSpec {
  ...
  const argv = [
    ...
    "--dangerously-load-development-channels", "server:mux-channel",
    "--add-dir", promptsDir(STATE_DIR),
    "--append-system-prompt-file", environmentMdPath(STATE_DIR),
    "--append-system-prompt-file", memoryPreamblePath,
  ]
  if (!coreReplyHookPresent) argv.push("--append-system-prompt-file", replyFallbackPath(STATE_DIR))
  argv.push(...pluginArgs)
  if (opts.model) argv.push("--model", opts.model)
  if (opts.effort) argv.push("--effort", opts.effort)
  if (opts.claudeSessionId) argv.push(opts.resume ? "--resume" : "--session-id", opts.claudeSessionId)
  if (existsSync(CLAUDE_HOOKS_SETTINGS_PATH)) argv.push("--settings", CLAUDE_HOOKS_SETTINGS_PATH)
  ...
  if (opts.rpcMcpConfig) argv.push("--strict-mcp-config", "--mcp-config", opts.rpcMcpConfig)
...
// Also reports whether mux-core's reply-conventions SessionStart hook is present ON
// DISK (not merely registered), so the caller knows whether the static reply
// fallback is needed.
function buildPluginArgs(sessionName: string, file?: string, pluginsDir?: string): { args: string[]; coreReplyHookPresent: boolean } {
