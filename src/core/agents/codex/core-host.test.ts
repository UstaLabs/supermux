...
function fakeChildFactory(options: { nativeId?: string; holdOpen?: boolean; holdClose?: boolean; failCloses?: number } = {}) {
  ...
  const factory = (gopts: CodexOptions, overrides: SessionConfiguration): AgentDriver => {
    codexCalls.push({ options: { ...gopts, env: { ...gopts.env } }, overrides: { ...overrides } })
    return {
      id: "codex",
      async open(ctx) {
  ...
  return {
    factory, opens, codexCalls, prompts,
...
async function makeHost(fake = fakeChildFactory()) {
  const stateDirectory = await tmp("codex-host-state-")
  const workdir = await tmp("codex-host-wd-")
  const host = createCodexCoreHost({ stateDirectory, driverFactory: fake.factory })
  hosts.push(host)
...
test("two sessions stay independent on one shared core", async () => {
  ...
  const wd2 = await tmp("codex-host-wd2-")
  const a = host.createAdapter({
    id: "sess-a", sessionName: "A", workdir, persistThreadId: async () => {}, env: { A: "1" },
  })
  const b = host.createAdapter({
    id: "sess-b", sessionName: "B", workdir: wd2, persistThreadId: async () => {}, env: { B: "2" },
  })
  await a.start()
  await b.start()
...
test("per-session command/args reach the driver factory", async () => {
  ...
  const adapter = host.createAdapter({
    id: "sess-cmd", sessionName: "cmd", workdir,
    persistThreadId: async () => {},
    env: { K: "1" },
    command: "/opt/codex",
    args: ["app-server", "-c", 'approval_policy="never"', "-c", 'plugins."x@mux".enabled=true'],
  })
  await adapter.start()
  expect(fake.codexCalls[0]?.options.command).toBe("/opt/codex")
  expect(fake.codexCalls[0]?.options.args).toEqual([
    "app-server", "-c", 'approval_policy="never"', "-c", 'plugins."x@mux".enabled=true',
  ])
  expect(fake.codexCalls[0]?.options.args?.join(" ")).not.toContain("model=")
})

test("factory codex() gets broker never/danger-full-access policy and cloned env, not sticky model defaults", async () => {
  const { host, workdir, fake } = await makeHost()
  ...
  const adapter = host.createAdapter({
    ...
    env, model: "gpt-5", effort: "high",
  })
  env.OPENAI_API_KEY = "mutated"
  await adapter.start()
  const first = fake.codexCalls[0]!
  expect(first.options.approvalPolicy).toBe("never")
  expect(first.options.sandbox).toBe("danger-full-access")
  expect(first.options.permissionPrompts).toBe("none")
  expect(first.options.env).toEqual({ OPENAI_API_KEY: "secret-live" })
  expect(first.options.model).toBeUndefined()
  expect(first.options.reasoningEffort).toBeUndefined()
  expect(persisted.length).toBe(1)
  const configureCall = fake.codexCalls.find((c) => c.overrides.model === "gpt-5")
  expect(configureCall?.overrides).toEqual({ model: "gpt-5", reasoningEffort: "high" })
})
...
test("duplicate live handle is rejected; replacement is allowed only after confirmed stop", async () => {
  ...
  const first = host.createAdapter({
    id: "same", sessionName: "one", workdir, persistThreadId: async () => {}, env: { K: "1" },
  })
  await first.start()
  expect(() => host.createAdapter({
    id: "same", sessionName: "two", workdir, persistThreadId: async () => {}, env: { K: "2" },
...
test("host.close stops every adapter and retains metadata for a new host resume", async () => {
  ...
  const adapter = host.createAdapter({
    ...
    persistThreadId: async (id) => { native = id },
    env: { X: "1" },
  })
  await adapter.start()
  ...
  const host2 = createCodexCoreHost({ stateDirectory, driverFactory: fake2.factory })
  hosts.push(host2)
  const resumed = host2.createAdapter({
    id: "sess-keep", sessionName: "keep", workdir,
    initialThreadId: native,
    persistThreadId: async () => {},
    env: { X: "1" },
  })
  await resumed.start()
...
test("single adapter.stop never closes sibling sessions", async () => {
  ...
  const wd2 = await tmp("codex-host-wd2-")
  const a = host.createAdapter({
    id: "left", sessionName: "L", workdir, persistThreadId: async () => {}, env: {},
  })
  const b = host.createAdapter({
    id: "right", sessionName: "R", workdir: wd2, persistThreadId: async () => {}, env: {},
  })
  await a.start()
  await b.start()
  await a.stop()
...
test("createAdapter is rejected while the host is closing", async () => {
  ...
  const adapter = host.createAdapter({
    id: "pending", sessionName: "p", workdir, persistThreadId: async () => {}, env: {},
  })
  const starting = adapter.start().catch(() => {})
  const closing = host.close()
  await tick()
  expect(() => host.createAdapter({
    id: "late", sessionName: "late", workdir, persistThreadId: async () => {}, env: {},
...
test("host.close retries a failed core close", async () => {
  ...
  const { host, workdir } = await makeHost(fake)
  const adapter = host.createAdapter({
    id: "retry", sessionName: "r", workdir, persistThreadId: async () => {}, env: {},
  })
  await adapter.start()
  await expect(host.close()).rejects.toThrow()
  expect(() => host.createAdapter({
    id: "after-fail", sessionName: "x", workdir, persistThreadId: async () => {}, env: {},
  })).toThrow(/closing or closed/)
  await host.close()
})
...
test("stale handle cannot start or resume after a replacement is live", async () => {
  const { host, workdir, fake } = await makeHost()
  const first = host.createAdapter({
    id: "same", sessionName: "one", workdir, persistThreadId: async () => {}, env: { K: "old" },
  })
  await first.start()
  await first.stop()
  const second = host.createAdapter({
    id: "same", sessionName: "two", workdir, persistThreadId: async () => {}, env: { K: "new" },
  })
  await second.start()
  ...
  expect(fake.codexCalls.at(-1)?.options.env).toEqual({ K: "new" })
})
...
test("stop-start race: terminal before cleanup; replacement only after confirmed close", async () => {
  ...
  await second.start()
  expect(fake.codexCalls.at(-1)?.options.env).toEqual({ K: "2" })
  await expect(first.start()).rejects.toThrow(/stopped/)
})
...
test("host closing forbids start and resume on remaining handles", async () => {
  ...
  const { host, workdir } = await makeHost(fake)
  const adapter = host.createAdapter({
    id: "sess", sessionName: "s", workdir, persistThreadId: async () => {}, env: {},
  })
  await adapter.start()
  const closing = host.close()
  await fake.waitCloseEntered()
...
test("sibling isolation: failed stop on one handle does not fence the other", async () => {
  ...
  const wd2 = await tmp("codex-host-wd2-")
  const a = host.createAdapter({
    id: "left", sessionName: "L", workdir, persistThreadId: async () => {}, env: { A: "1" },
  })
  const b = host.createAdapter({
    id: "right", sessionName: "R", workdir: wd2, persistThreadId: async () => {}, env: { B: "2" },
  })
  await a.start()
  await b.start()
...
test("failed start leftover is cleaned by stop; sibling stays alive; replacement after confirmed cleanup", async () => {
  ...
  const host = createCodexCoreHost({ stateDirectory: dirState, driverFactory: factory })
  hosts.push(host)
  const sibling = host.createAdapter({
    id: "sib", sessionName: "sib", workdir: wd2, persistThreadId: async () => {}, env: { S: "1" },
  })
  await sibling.start()
  const a = host.createAdapter({
    id: "same-id", sessionName: "s", workdir: wd, persistThreadId: async () => {}, env: { K: "1" },
  })
  ...
  expect(() => host.createAdapter({
    id: "same-id", sessionName: "s2", workdir: wd, persistThreadId: async () => {}, env: { K: "2" },
  })).toThrow(/already live/)
...
test("host.close aborts driver.open that only exits on ctx.signal and then settles", async () => {
  ...
  hosts.push(host)
  const adapter = host.createAdapter({
    id: "open-wait", sessionName: "ow", workdir: wd, persistThreadId: async () => {}, env: {},
  })
  const starting = adapter.start().catch(() => {})
...
test("pending open is aborted by host shutdown", async () => {
  ...
  const { host, workdir } = await makeHost(fake)
  const adapter = host.createAdapter({
    id: "slow", sessionName: "s", workdir, persistThreadId: async () => {}, env: {},
  })
