...
function attachCursorNormalizer(runtime: AgentRuntime): AgentRuntime {
    ...
    async open(ctx) {
      ...
      const runtime: AgentRuntime = {
        ...
        capabilities: {
          ...
          configure: options.configure === true, history: false,
        },
      ...
      return attachCursorNormalizer(runtime)
    },
  }
...
const dirs: string[] = []
const hosts: Host[] = []
const adapters: CoreCursorAdapter[] = []
...
async function harness(fake: { driver: AgentDriver } | AgentDriver = fakeAgentDriver()) {
  ...
  const workdir = await mkdtemp(join(tmpdir(), "cursor-wd-"))
  const stateDirectory = await mkdtemp(join(tmpdir(), "cursor-core-"))
...
function listen(adapter: CoreCursorAdapter) {
  const events: any[] = []
...
test("existing core record with mismatched agent/cwd/native id is rejected", async () => {
  ...
  await first.start()
  await first.stop()
  const other = await mkdtemp(join(tmpdir(), "cursor-wd-"))
  dirs.push(other)
...
test("native initialize metadata is applied; replayed assistant chat is skipped", async () => {
  const fake = fakeAgentDriver()
  const { host, workdir } = await harness(fake)
  const adapter = makeAdapter(host, {id: "sess-1", sessionName: "s1", workdir,
    persistSessionId: async () => {},
  ...
  fake.emit({
    protocol: "native",
    value: { method: "initialize", params: { _meta: { modelState: { availableModels: [{ modelId: "cursor-4.5" }] } } } },
  })
  fake.emit({
    protocol: "native",
    replay: true,
    value: { type: "assistant", message: { role: "assistant", content: [{ type: "text", text: "old history" }] } },
  })
  await flush()
  expect(adapter.availableModels.map((m) => m.modelId)).toEqual(["cursor-4.5"])
...
test("buffers assistant deltas and flushes before tool start and turn end", async () => {
  ...
  await tick()
  fake.emit({ protocol: "native", value: { type: "assistant", message: { role: "assistant", content: [{ type: "text", text: "Step 1." }] } } })
  fake.emit({ protocol: "native", value: { type: "tool_call", subtype: "started", call_id: "c1", tool_call: { writeToolCall: { args: { path: "a.ts" } } } } })
  fake.emit({ protocol: "native", value: { type: "assistant", message: { role: "assistant", content: [{ type: "text", text: "Done." }] } } })
  fake.emit({ protocol: "native", value: { type: "result", subtype: "success" } })
  fake.completePrompt()
  await sent
  await flush()
...
test("active-turn delayed close emits turn-complete only after native close", async () => {
  ...
  const sent = adapter.send("hi")
  for (let i = 0; i < 20 && fake.prompts.length === 0; i++) await tick()
  fake.emit({ protocol: "native", value: { type: "assistant", message: { role: "assistant", content: [{ type: "text", text: "partial" }] } } })
  await tick()
...
test("active-turn failed close keeps pending buffer and does not fake idle", async () => {
  ...
  const sent = adapter.send("hi")
  for (let i = 0; i < 20 && fake.prompts.length === 0; i++) await tick()
  fake.emit({ protocol: "native", value: { type: "assistant", message: { role: "assistant", content: [{ type: "text", text: "held" }] } } })
  await tick()
  await expect(adapter.stop()).rejects.toThrow(/native close failed/)
  expect(events.filter((e) => e.kind === "turn-complete")).toHaveLength(0)
  expect(events.filter((e) => e.kind === "assistant-message")).toHaveLength(0)
...
function cursorFrameToUpdate(frame: unknown) {
  return { protocol: "native" as const, value: frame }
}

test("replays real cursor-turn.ndjson through Core normalizer into broker events", async () => {
  const fake = fakeAgentDriver()
  const { host, workdir } = await harness(fake)
  ...
  await waitUntil(() => fake.prompts.length === 1)
  const raw = await readFile("packages/supermux-core/tests/fixtures/real/cursor-turn.ndjson", "utf8")
  for (const line of raw.split("\n")) {
    if (!line.trim()) continue
    fake.emit(cursorFrameToUpdate(JSON.parse(line)))
  }
  fake.completePrompt()
  await sent
  await flush()
