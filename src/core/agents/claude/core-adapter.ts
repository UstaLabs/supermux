...
export type CoreClaudeAdapterOpts = {
  handle: HostHandle
...
/** Broker-facing Claude adapter that talks to a host-owned supermux-core instance.
 * Does not spawn a native child, subclass ClaudeAdapter, or close the shared Core. */
export class CoreClaudeAdapter extends EventEmitter implements AgentAdapter {
  ...
  private readonly reregister: CoreClaudeAdapterOpts["reregister"]
  ...
  constructor(opts: CoreClaudeAdapterOpts) {
    super()
  ...
  async interrupt(): Promise<void> {
    ...
    if (result.status === "unconfirmed") {
      const error = new Error("claude interrupt is unconfirmed; the turn may still be running")
      this.surfaceFailure(error, { completeTurn: false })
      throw error
    }
  ...
  private assertCompatibleRecord(record: { agent: string; cwd: string; agentSessionId: string }): void {
    if (record.agent !== "claude") {
      throw new Error(`core session ${this.id} is agent ${record.agent}, expected claude`)
    }
  ...
  private assertLive(epoch: number): void {
    if (this.stopped || epoch !== this.inputEpoch || !this.session) {
      throw new Error("claude adapter is stopped")
    }
  }

  private requireSession(): Session {
    if (!this.session) throw new Error("claude session not initialized")
    return this.session
  }
  ...
  private ingestInitialize(params: unknown): void {
    type Command = CoreClaudeAdapter["availableCommands"][number]
  ...
  private armStall(): void {
    ...
    this.stallTimer = setTimeout(() => {
      ...
      const error = new Error(
        `claude produced no response within ${Math.round(this.stallTimeoutMs / 1000)}s — the request appears stalled; please try again`,
      )
      this.surfaceFailure(error, { completeTurn: false })
