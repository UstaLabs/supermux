...
test("ignores text blocks and unknown top-level types", () => {
  expect(parseTranscriptLine(JSON.stringify({ type: "assistant", timestamp: TS, message: { content: [{ type: "text", text: "hi" }] } }), WD)).toEqual([])
  expect(parseTranscriptLine(JSON.stringify({ type: "system", timestamp: TS }), WD)).toEqual([])
})

test("defensive: never throws on malformed input", () => {
  expect(parseTranscriptLine("not json", WD)).toEqual([])
...
test("parses the interrupt marker (generation) into an interrupt event", () => {
  ...
  const out = parseTranscriptLine(line, WD)
...
test("parses the tool-use interrupt marker too", () => {
  ...
  expect(parseTranscriptLine(line, WD)[0]!.kind).toBe("interrupt")
})
...
test("ordinary assistant text is NOT an interrupt", () => {
  ...
  expect(parseTranscriptLine(line, WD)).toHaveLength(0)
})

test("absolute file_path is relativized against workdir", () => {
  const line = JSON.stringify({
    type: "assistant", timestamp: TS,
...
test("absolute file_path outside workdir stays absolute", () => {
  ...
  const [ev] = parseTranscriptLine(line, WD)
  expect(ev).toMatchObject({ kind: "tool", tool: "Read", title: "Read: /etc/hosts" })
})

test("undefined workdir -> path unchanged", () => {
  const line = JSON.stringify({
    type: "assistant", timestamp: TS,
    message: { content: [{ type: "tool_use", id: "t1", name: "Read", input: { file_path: "/src/main.ts" } }] },
  })
  const [ev] = parseTranscriptLine(line, undefined)
  expect(ev).toMatchObject({ kind: "tool", tool: "Read", title: "Read: /src/main.ts" })
})
