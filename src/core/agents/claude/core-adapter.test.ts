...
import { createClaudeNormalizer } from "../../../../packages/supermux-core/src/claude/normalize.js"
import { CoreClaudeAdapter } from "./core-adapter"
import { createClaudeCoreHost } from "./core-host"

function attachClaudeNormalizer(runtime: AgentRuntime): AgentRuntime {
...
function fakeAgentDriver(options: { configure?: boolean; nativeId?: string } = {}) {
  ...
  const driver: AgentDriver = {
    ...
    async open(ctx) {
      ...
      return attachClaudeNormalizer(runtime)
    },
  }
...
const dirs: string[] = []
const hosts: Host[] = []
const adapters: CoreClaudeAdapter[] = []

async function harness(fake: { driver: AgentDriver } | AgentDriver = fakeAgentDriver()) {
  const workdir = await mkdtemp(join(tmpdir(), "claude-wd-"))
  const stateDirectory = await mkdtemp(join(tmpdir(), "claude-core-"))
...
function makeAdapter(host: Host, opts: {
  ...
  const adapter = new CoreClaudeAdapter({
    handle,
...
function listen(adapter: CoreClaudeAdapter) {
  const events: any[] = []
...
test("existing core record with mismatched agent/cwd/native id is rejected", async () => {
  ...
  await first.start()
  await first.stop()
  const other = await mkdtemp(join(tmpdir(), "claude-wd-"))
  dirs.push(other)
