...
<template>
  <DialogRoot :open="props.open" @update:open="(v) => emit('update:open', v)">
    <DialogPortal>
      ...
      >
        <h3 class="font-semibold text-base">Remove proxy</h3>
        <p class="text-sm text-muted-foreground mt-2">
          Remove <span class="font-mono font-semibold text-foreground">{{ props.domain }}</span>? The proxied app will no longer be accessible from the web.
        </p>
        <div class="flex gap-3 mt-5 justify-end">
          <Button variant="outline" size="sm" @click="emit('update:open', false)">Cancel</Button>
          <Button variant="destructive" size="sm" @click="emit('confirm')">Remove</Button>
        </div>
      </DialogContent>
    </DialogPortal>
  </DialogRoot>
