...
const props = defineProps<{ open: boolean; sessionName: string }>()
...
<template>
  <DialogRoot :open="props.open" @update:open="(v) => emit('update:open', v)">
    <DialogPortal>
      ...
      >
        <h3 class="font-semibold text-base">Kill session?</h3>
        <p class="text-sm text-muted-foreground mt-2">
          This will terminate <span class="font-medium text-foreground">{{ props.sessionName }}</span>. This cannot be undone.
        </p>
        <div class="flex gap-3 mt-5 justify-end">
          <Button variant="outline" size="sm" @click="emit('update:open', false)">Cancel</Button>
          <Button variant="destructive" size="sm" @click="emit('confirm')">Kill</Button>
        </div>
      </DialogContent>
    </DialogPortal>
  </DialogRoot>
