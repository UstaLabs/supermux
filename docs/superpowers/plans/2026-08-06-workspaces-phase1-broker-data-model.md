...
export class WorkspaceStore {
  ...
  /**
   * Views in creation order. The tiebreak is rowid, not id: created_at has
   * millisecond resolution, so two views added in the same millisecond would
   * otherwise order by random UUID — and "the oldest chat view" is what picks a
   * workspace's primary session. rowid is monotonic per insert.
   */
