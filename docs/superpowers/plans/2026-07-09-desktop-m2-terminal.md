...
- [x] **Step 1:** Added `SM_TERM_INPUT` env hook (Main.kt) — format `<session-name>:<text>` (with `\n\r\t` unescaped); after snapshot + a 5s settle it opens the session's SCRATCH terminal via `app.connectTerminal(id, "main")` (which CANNOT construct a kind=agent client → scratch-only enforced in code), drives the run-loop, waits for CONNECTED, and writes the bytes. Same doc pattern as SM_SMOKE_SEND. Committed before the checklist (`feat(desktop): SM_TERM_INPUT headless hook`).
- [x] **Step 2:** Live checklist (Xvfb :77, live broker :9898, throwaway `talebe-dummy-excel`; screenshots `/home/ahmet/.cache/m2v-<n>-<slug>.png`):
  1. **PASS** — `panes.terminal=true` via ui-state → tab strip (`main ×` + `+`) above a live `$` shell prompt in Geist Mono on the branded bg. `m2v-1-scratch-render.png`.
  2. **PASS** — `SM_TERM_INPUT='talebe-dummy-excel:echo m2-verify-7351 && date\n'` (hook logged `wrote 28 chars`) → command + output (`m2-verify-7351`, `Thu Jul 9 06:27:06 PM +03 2026`) rendered in JediTerm. Full JediTerm→WS→tmux→WS→JediTerm loop. `m2v-2-roundtrip.png`. NB: hook opens a *second* WS client on tmux "main"; the broker serves the persistent screen on attach, so the rendered output is captured on the pane's fresh re-attach (single-client live render is proven by the prompt rendering on connect + write tests).
  3. **PASS** — `seq 1 200` via the hook → terminal tail (161–200 + prompt) rendered, proving scrollable output. `m2v-3-scroll.png`. Wheel/mouse reporting is JediTerm-native + bytecode-verified (Task 4) — no pointer available headless, documented substitution.
  4. **PASS (decisive)** — created a 2nd scratch terminal server-side (`verify2`, raw-socket WS to `/ws/term`) → strip hydrated BOTH `main ×` and `verify2 ×` from `listTerminals`, proving live hydration. Add/close = UI-test-covered (7 TerminalTabsTest). `m2v-4-tabs.png`.
  5. **PASS** — ui-state `native:{talebe:true}` → Native pill active, live Claude TUI (agent PTY, kind=agent) rendered; view-only, no input. `m2v-5-agent-native.png`.
  6. **SUBSTITUTED** — pointer-driven pane toggle impossible headless (no xdotool). Keep-alive is test-verified: `KeepAlivePanelTest.content_survives_hide_show_without_remount_or_disposal` (+2 size tests) + `SessionDetailTest.togglingSwapsContentButKeepsChatInTree`. A persisted-panes relaunch is a fresh process (not an in-process toggle) so it can't exercise the live keep-alive path — noted honestly.
  7. **PASS (via prune path)** — app in Native on talebe → `DELETE /sessions/<id>` → app handled the kill gracefully: `rx SessionRemoved`, session left the fleet, Native panel + client disposed cleanly, `native` map cleared, no crash, fell back to "select a session". `m2v-7a-prekill.png` / `m2v-7-agent-exit.png`. FINDING: the session-DELETE drives the *session-removal prune* (main WS), which disposes the SessionDetail + exit collector before the agent term WS's `{"type":"exit"}` frame is processed — so the isolated exit-FRAME → Native→Chat fallback (T7 fix) was pre-empted by prune (both reach the correct end state). The exit-frame trigger stays UI-test-verified (`SessionDetailTest.onExitFlipsBackToChatAndClearsNativeView`); added an observability log at the exit collector (`feat(desktop): log agent-PTY exit frame`).
  8. **PASS** — suites fresh (`--rerun-tasks`) at HEAD: `:desktop:test` 129 / 0 fail, `:shared:jvmTest` 292 / 0 fail, `:android:compileDebugKotlin` green.
  9. **PASS (test-verified)** — predictive echo: 23 shared `PredictiveEchoTest` + 6 desktop `PredictionAdapterTest`. Visual dim-echo needs >40ms RTT (impossible on localhost) — feel-test deferred to a real remote link (same status as Android/iOS).
  10. **DONE** — killed app + Xvfb; reset ui-state to a clean default (empty `native`, `selectedId:null`). Broker artifacts created: session `talebe-dummy-excel` killed/archived (sanctioned), which took its `verify2` scratch terminal with it (fleet 23→22). No devices created (reused paired auth).
- [x] **Step 3:** Fixes committed (SM_TERM_INPUT hook + exit-frame log). Checkboxes updated; committing `docs(desktop): M2 plan executed`.
- [x] **Step 4:** Report delivered in the task reply.
