...
Known wall (from Task 5's review): `Editor/EditorPane.swift` has an unclaimed UIKit subsystem — the software-keyboard dismiss feature (`UIResponder.keyboardWillShow/Hide` notifications ~lines 73-76, the `keyboardDismissOverlay` ~lines 297-320, and `webView?.endEditing(true)`). No earlier task owns it. When it surfaces here, wrap the ENTIRE subsystem (state + notifications + overlay + endEditing) in `#if os(iOS)` — it's meaningless on a hardware-keyboard Mac — rather than patching individual error lines. **Same sweep must include** (from Task 7's review): `Editor/EditorWebView.swift`'s `Coordinator.dismissKeyboard()` (~line 257, another `webView?.endEditing(true)` — same feature, same `#if os(iOS)` treatment including its call sites), and the stale comment at `Editor/EditorHost.swift:25` that still says `EditorWebView.updateUIView` (renamed to `updatePlatformView` in Task 7). While in `EditorWebView.swift`, also add the discoverability twin `#if os(macOS) static func dismantleNSView(_ view: WKWebView, coordinator: Coordinator) {} #endif` next to the existing `dismantleUIView` no-op, carrying the same persistent-webview comment (behaviorally identical either way; makes the intent greppable on both platforms).

Known wall (from Task 11): `Push/PushManager.swift` `didReceiveRemoteNotification(_:)` (~line 118) still returns `UIBackgroundFetchResult` (UIKit-only, unconditioned) — deliberately left as the only mac blocker in that file (typecheck-verified). Fix here: give the method a platform-neutral return (e.g. an internal enum or Bool mapped to `UIBackgroundFetchResult` inside the iOS delegate branch) or `#if`-split the signature; the iOS delegate forward must keep returning the real fetch result.

Task 8's implementer flagged its own top mac compile-risk suspects (never type-checked yet) — check these first when the chat files hit the wall: (1) `PlatformFont.preferredFont(forTextStyle:)` single-arg in shared MarkdownView code (AppKit may require `options:`); (2) `NSTextField` label API surface in the composer twin (`lineBreakMode`/`maximumNumberOfLines`, `labelWithString:` on a subclass); (3) `#selector(NSResponder.insertNewline(_:))`/`#selector(NSText.paste(_:))` resolution + the `validateUserInterfaceItem` override signature; (4) postfix `#if os(iOS)` chains in ChatPane/NewSessionView on the mac parse path; (5) runtime, not compile: the composer/markdown NSTextViews use `layoutManager` (forces TextKit-1 compatibility) — eyeball once the app runs.
...
**Display-subsystem first-run checklist (from Task 9's quality review — run during Step 3's launch smoke):** open a display → both surfaces render (VNC Metal + scrcpy video; a crash here means the parameterless-init→init(frame:) funnel assumption failed); live-resize the viewer while streaming (no sustained blur); drag the window Retina↔non-Retina during a VNC stream (crispness recovers immediately — validates the backing-properties hook); blow the scrcpy viewer past the decoded frame size on Retina (if soft, mirror the Metal contentsScale handling); the viewer sheet opens ≥1000×700 and resizes; keyboard toggle on→click-away→toggle reflects off (no wedged state); ESC with keyboard ON reaches the remote, with keyboard OFF dismisses the sheet (by design). **Known v1 limitation (document, don't fix here): mac key capture composes no dead-keys/IME** — accented Latin (é/ñ) and CJK-into-scrcpy won't type on mac (iOS's UITextField composed for free); proper fix is NSTextInputClient adoption — post-v1 follow-up.

**Push first-run checks (from Task 11's quality review — some need Task 16/TestFlight):** tap a delivered notification while the app runs with ZERO open windows (Cmd+W, not quit) — confirm a window opens/foregrounds on the tapped session rather than `pendingSessionId` silently setting; cold-launch tap (app fully quit) — confirm the UN delegate is assigned before the tap delivers and routing works; note whether a fully-quit mac app receives the silent bootstrap push at all (likely not — `registerIfPaired()` self-heals on next launch; document as expected).

**Dictation-subsystem first-run checklist (from Task 10's quality review — run during Step 3's launch smoke):** grant mic + speech-recognition permission on a genuinely fresh install and confirm the FIRST dictation attempt actually starts (not `.unavailable`) — mac skips the `session.setActive(true)` nudge iOS relies on to settle the input route, so this exact path has never run on real hardware; if the on-device SpeechAnalyzer language model isn't resident yet, confirm the asset download completes inside the sandboxed container rather than stalling; on a Mac with no microphone (or Mic disabled — plausible on App Review hardware), confirm `AudioRecorder.start()` and `SpeechDictation.start()` resolve to `.denied`/`.failed`/`.unavailable` cleanly rather than hanging; confirm (expected, no fix) other audio is NOT ducked during dictation (no sandboxed-mac `.duckOthers` equivalent); unplug/reconnect the input device mid-dictation and confirm no hang — neither platform handles route-change notifications today, and desktop peripherals make it likelier (post-v1 follow-up candidate).
...
## Post-plan follow-ups discovered during execution (OUTSIDE this plan's scope)
