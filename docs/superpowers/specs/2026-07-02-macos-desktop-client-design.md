...
- **Workspace layout:** the iPad multi-pane workspace adapts to a `NavigationSplitView` — sessions sidebar, chat center, terminal/editor/display panes. Mac-native additions: open-session-in-new-window (`WindowGroup` scenes; each detached window opens its own `BrokerSession`/WS — the **web-tab model**, where every window is an independent broker client and the broker fans out to N clients; simpler than hoisting a shared app-level store, and the extra per-window snapshot is the same cost as opening a second browser tab), a real menu bar (⌘N new session, ⌘W close window, pane-focus and send shortcuts), resizable splits with persisted sizes.
...
- **Multi-window:** each detached session window is an independent broker client with its own `BrokerSession`/WS (the **web-tab model** — like opening the session in a second browser tab). Windows don't share a store; the broker already fans out to N clients, so this is simpler than hoisting a shared app-level session and avoids the unpair/restore lifecycle coupling that sharing would introduce. Closing one window tears down only that window's WS — never the main workspace's or another session window's.
