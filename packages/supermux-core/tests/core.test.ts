...
describe("session lifecycle", () => {
  ...
  test("failed resume preserves saved history and does not retry new", async () => {
    ...
    const next = createCore({ stateDirectory, agents: [{ id: "test", async open(ctx) { requests.push(ctx); throw new Error("history gone") } }] })
    cores.push(next)
  ...
  test("close waits for an opening session and releases the late runtime", async () => {
    ...
    const creating = core.sessions.create({ agent: "slow", cwd: tmpdir() }).catch(e => e)
  ...
  test("fork, detach and steer are explicit unsupported operations", async () => {
    ...
    const s = await core.sessions.create({ agent: "test", cwd: tmpdir() })
    await expect(s.fork()).rejects.toMatchObject({ code: "unsupported_operation" })
...
describe("input ownership", () => {
  ...
  test("profiles resolve for auth and sessions without persisting their environment", async () => {
    ...
    const s = await core.sessions.create({ agent: "test", cwd: tmpdir(), authProfile: "work" })
    ...
    await expect(core.sessions.create({ agent: "test", cwd: tmpdir(), authProfile: "missing" })).rejects.toMatchObject({ code: "invalid_auth_profile" })
  })
...
test("failed shutdown retains state ownership and can be retried", async () => {
  ...
  await expect(core.close()).rejects.toThrow()
  const other = createCore({ stateDirectory, agents: [] })
  cores.push(other)
  await expect(other.sessions.list()).rejects.toThrow()
  await core.close()
  const next = createCore({ stateDirectory, agents: [] })
  cores.push(next)
...
test("failed cleanup of a late-opening runtime stays owned until retry", async () => {
  ...
  const creating = core.sessions.create({ agent: "late", cwd: tmpdir() }).catch(e => e)
  await entered.promise
  const closing = core.close()
  opening.resolve()
...
test("fork persists lineage and uses native checkpoint identity", async () => {
  ...
  const {core}=await setup(d.driver)
  const parent=await core.sessions.create({agent:'test',cwd:tmpdir()})
  const child=await parent.fork({at:{nativeTurnId:'turn-1'}})
...
test("fork excludes new source input until native fork settles",async()=>{
  ...
  const {core}=await setup(d.driver)
  const source=await core.sessions.create({agent:'test',cwd:tmpdir()})
  const fork=source.fork();await entered.promise
...
test("fork rejects a native runtime reusing its parent's identity",async()=>{
  ...
  const {core}=await setup(d.driver),source=await core.sessions.create({agent:'test',cwd:tmpdir()})
  await expect(source.fork()).rejects.toMatchObject({code:'fork_identity_unchanged'})
