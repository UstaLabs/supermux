...
describe("optional configuration and history", () => {
  test("unsupported drivers reject configure and history explicitly", async () => {
    ...
    const session = await core.sessions.create({ agent: "plain", cwd: tmpdir() })
  ...
  test("applies, persists, and restores configuration on reopen", async () => {
    ...
    const session = await core.sessions.create({ agent: "test", cwd: tmpdir() })
    expect(session.configuration()).toEqual({})
  ...
  test("in-flight configuration blocks send and fork", async () => {
    ...
    await expect(session.fork()).rejects.toMatchObject({ code: "session_busy" })
...
describe("initial configuration before native open", () => {
  test("create forwards cloned requested configuration on open and persist", async () => {
    ...
    const pending = core.sessions.create({ agent: "test", cwd: tmpdir(), configuration })
  ...
  test("invalid create configuration never opens native", async () => {
    const d = configurableDriver()
    const { core } = await setup(d.driver)
    await expect(core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: "" } }))
      .rejects.toMatchObject({ code: "invalid_input" })
    await expect(core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { temperature: "1" } as SessionConfiguration }))
      .rejects.toMatchObject({ code: "invalid_input" })
    await expect(core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: ["x"] as unknown as SessionConfiguration }))
      .rejects.toMatchObject({ code: "invalid_input" })
  ...
  test("create empty or omitted configuration omits open and disk fields", async () => {
    const d = configurableDriver()
    const { core } = await setup(d.driver)
    const omitted = await core.sessions.create({ agent: "test", cwd: tmpdir() })
    expect(d.opens[0]?.configuration).toBeUndefined()
    expect((await core.sessions.get(omitted.id))?.configuration).toBeUndefined()
    const empty = await core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: {} })
    expect(d.opens[1]?.configuration).toBeUndefined()
    expect((await core.sessions.get(empty.id))?.configuration).toBeUndefined()
    const cleared = await core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: undefined } })
  ...
  test("resume no-options keeps persisted configuration on open", async () => {
    const d = configurableDriver()
    const { core, stateDirectory } = await setup(d.driver)
    const session = await core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: "keep" } })
    await session.close()
    await core.close()
    const next = createCore({ stateDirectory, agents: [d.driver] })
    cores.push(next)
  ...
  test("resume patch undefined values clear keys for open and disk", async () => {
    const d = configurableDriver()
    const { core } = await setup(d.driver)
    const session = await core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: "keep", reasoningEffort: "low" } })
    await session.close()
  ...
  test("no-options in flight plus override is busy and keeps old config", async () => {
    const d = configurableDriver()
    const { core } = await setup(d.driver)
    const session = await core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: "old" } })
    await session.close()
    const held = d.holdOpen()
    const restoring = core.sessions.resume(session.id)
    await held.entered.promise
  ...
  test("failed native open on override resume keeps original record", async () => {
    ...
    const next = createCore({ stateDirectory, agents: [failing.driver] })
    cores.push(next)
  ...
  test("create put that commits then rejects removes the new record", async () => {
    ...
    await expect(core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: "new" } })).rejects.toThrow("post-write failure")
    expect(await core.sessions.list()).toHaveLength(0)
    expect(d.closed).toBe(1)
  })
  ...
  test("non-configurable runtime rejects nonempty create and override resume", async () => {
    ...
    await expect(core.sessions.create({ agent: "test", cwd: tmpdir(), configuration: { model: "x" } }))
      .rejects.toMatchObject({ code: "unsupported_operation" })
    expect(await core.sessions.list()).toHaveLength(0)
    expect(closed).toBe(1)
    const ok = await core.sessions.create({ agent: "test", cwd: tmpdir() })
    await ok.close()
    await core.close()
    const d = configurableDriver()
    const mid = createCore({ stateDirectory, agents: [d.driver] })
    cores.push(mid)
    ...
    closed = 0
    opens.length = 0
    const next = createCore({ stateDirectory, agents: [plain] })
    cores.push(next)
  ...
  test("resume identity change with override leaves original disk config", async () => {
    ...
    const next = createCore({ stateDirectory, agents: [wrong.driver] })
    cores.push(next)
