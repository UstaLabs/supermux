...
const driver = (env = {}, extra = {}) => acp({id:'fixture',command:process.execPath,args:[fixture],env,setupTimeoutMs:3000,shutdownTimeoutMs:40,classifyActivity,...extra})
...
test('create, prompt, preserve updates, and close process', async()=>{
 ...
 expect(runtime.agentSessionId).toBe('agent-1'); expect(runtime.capabilities).toEqual({resume:true,steer:false,fork:false,detach:false});
 ...
 const pid = Number(await readFile(pidFile,'utf8')); await runtime.close(); expect(()=>process.kill(pid,0)).toThrow();
...
test('auth honors profile env, exposes methods, rejects terminal and failed authentication',async()=>{
 ...
 const r=await d.open(context(ctx)); await r.close();
...
test('permission cancellation settles uncooperative callback and prompt',async()=>{
 ...
 await seen; controller.abort(); await r.interrupt(); expect(await result).toEqual({stopReason:'cancelled'});expect(permissionSignal!.aborted).toBe(true);await r.close();
})
test('close settles outstanding prompt and escalates stubborn child',async()=>{
 ...
 await new Promise(r=>setTimeout(r,20)); await r.close();expect(await result).toBeInstanceOf(Error);
})
...
test('resume takes precedence over load without replay', async()=>{
 ...
 const r=await driver({FIXTURE_MODE:'resume',TRACE:trace}).open(context({resumeId:'old',onUpdate:u=>updates.push(u)}));await r.close();
...
test('autonomous native begin drives core running, queue, reject, and configure busy', async () => {
  ...
  try {
    const session = await core.sessions.create({ agent: 'fixture', cwd: process.cwd() })
  ...
  } finally { await core.close(); await rm(stateDirectory, { recursive: true, force: true }) }
})
