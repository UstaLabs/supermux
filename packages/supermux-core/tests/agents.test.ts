...
function driver(env: Record<string, string> = {}, extra: Parameters<typeof grok>[0] = {}) {
  return grok({ command: process.execPath, commandArgs: [fixture], env, setupTimeoutMs: 4000, shutdownTimeoutMs: 40, ...extra })
}
...
test('defaults never auto-approve and keep library noLeader', async () => {
  expect(() => grok({ authPath: 'relative' })).toThrow('absolute')
  expect(() => grok({ reasoningEffort: 'max' as 'low' })).toThrow('reasoningEffort')
  expect(() => grok({ model: '' })).toThrow('model')
  expect(() => grok({ commandArgs: 'fixture' as unknown as string[] })).toThrow('commandArgs')
  const { r, dir, lines } = await traced()
  try {
    expect(r.capabilities).toEqual({ resume: true, steer: false, fork: false, detach: false, configure: true, history: false })
...
test('failed restart leaves wrapper retryable and core rollback restores previous argv', async () => {
  ...
  cores.push(core)
  const session = await core.sessions.create({ agent: 'grok', cwd: process.cwd() })
...
test('failed child close keeps ownership so repeated close can retry', async () => {
  let attempts = 0
  const r = await grok({ command: 'grok' }, () => mockChild(async () => {
    attempts++
    if (attempts === 1) throw new Error('cleanup failed')
  })).open(ctx())
  await expect(r.close()).rejects.toThrow('cleanup failed')
...
test('candidate onExit before publish does not fail the current session', async () => {
  const exits: Error[] = []
  let opens = 0
  const r = await grok({ command: 'grok' }, () => mockChild(async () => {}, context => {
    opens++
...
test('stale child activity after configure is generation-gated', async () => {
  ...
  const r = await grok({ command: 'grok' }, () => ({
    id: 'grok',
    auth: { methods: async () => [], login: async () => {} },
    async open(context) {
      opens++
...
test('initial onExit candidate close failure stays owned until core.close retries', async () => {
  ...
  const core = createCore({
    stateDirectory,
    agents: [grok({ command: 'grok' }, () => mockChild(async () => {
      closes++
      if (!allowClose) throw new Error('cleanup blocked')
    }, context => { context.onExit(new Error('candidate died during setup')) }))],
  })
  cores.push(core)
  await expect(core.sessions.create({ agent: 'grok', cwd: stateDirectory })).rejects.toThrow()
...
test('failed owned open returns close-only facade until cleanup succeeds', async () => {
  ...
  await expect(r.close()).rejects.toThrow('cleanup blocked')
...
test('prepublication activity overflow fails setup without publishing notices', async () => {
  const activity: any[] = []
  const exits: Error[] = []
  await expect(grok({ command: 'grok' }, () => mockChild(async () => {}, context => {
