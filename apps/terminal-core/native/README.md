...
8. **Wrapper** (`WRAPPER_SOURCES`, `ST_ABI_VERSION=1`, must match the header):
   ...
   **macOS dylibs are linked only on a macOS host**, by `xcrun clang++`
   (ld64, system libc++, `-mmacosx-version-min=13.0` = Zig's default for `aarch64-macos`/`x86_64-macos`): Zig's self-hosted
...
Every platform implements `TerminalEngine` over the st_* ABI + `ViewportCodec`;
status codes map to exceptions in one shared place (`NativeStatus.kt`:
`INVALID_ARGUMENT` → `IllegalArgumentException`, `LIMIT`/`OUT_OF_MEMORY`/
`INTERNAL` → `TerminalNativeException(status)`, `REJECTED` paste → `false`).
**Threading:** each engine holds a lock + closed flag; every call takes the
lock and checks the flag, so the same handle is never used concurrently and
never after `st_destroy`. After `close()` every call throws
`IllegalStateException`; `close()` is idempotent. Different engines never
contend.

**Startup errors** are `TerminalEngineUnavailableException` with a `reason`:
`UNSUPPORTED_PLATFORM` (OS/CPU without a packaged library),
`MISSING_BINARY` (resource / jniLibs entry absent), `CORRUPT_BINARY` (bytes
...
- **JNI (Android + desktop JVM)**: `src/jvmAndAndroidMain` (a `jvmAndAndroid`
  ...
  external fun`, 1:1 with `native/src/terminal_jni.c`) and
  `NativeTerminalEngine`.
  - **Android** (`androidMain`): `System.loadLibrary("supermux_terminal_jni")`
    after checking `Build.SUPPORTED_ABIS` against the packaged `arm64-v8a` /
    `x86_64`; then `st_abi_version()`. Gradle stages
    `build/native/android-{arm64,x64}/lib/libsupermux_terminal_jni.so`
    (sha256 checked against `manifest.json`) into the AAR's `jni/<abi>/` via
    the AGP variant API (`stageAndroidJniLibs`).
  - **Desktop JVM** (`jvmMain/JvmNativeLibrary.kt`): maps `os.name`/`os.arch`
    to `linux|macos|windows-x64|arm64`, reads
    `/dev/supermux/terminal/native/<key>/native.properties` (library, sha256,
    size, abi, version — generated by `stageJvmNativeResources` from the
    build manifest), checks ABI, hashes the resource bytes, extracts them to
    `<cache>/supermux-terminal/<version>/<sha256>/` (`-Dsupermux.terminal.cacheDir`,
...
- **iOS** (`iosMain`, `src/nativeInterop/cinterop/terminal.def`): cinterop
  ...
  for the synchronous st_* call, output buffers are copied with `readBytes`
