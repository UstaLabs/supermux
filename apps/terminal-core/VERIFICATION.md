...
|---|---|---|---|---|
| `:terminal-core:jvmTest` | linux-x64 (JDK 17) | **91** | 0 | 2026-09-23 (Plan 2 Task 4: +2 renderer-lease tests) |
...
Linux 91 = TerminalSessionTest 28, JvmNativeLoaderTest 13, EngineContractTest 12,
...
WasmRuntimeTest 14 — that run predates the two renderer-lease tests, which are in
commonTest and would make it 80 when re-run.
...
2. **macOS/iOS Kotlin suites are one task behind**: the Mac last ran at the Task-5 tree, so the
   `TerminalSession` owner (31 tests) has never executed on Apple targets. Re-run
