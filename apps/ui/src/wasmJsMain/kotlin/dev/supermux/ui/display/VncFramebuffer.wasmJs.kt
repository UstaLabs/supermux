...
/**
 * The wasm actual: the same Skia upload as desktop. A flat BGRA byte buffer mutated in place by
 * [VncFrameOps], installed into a long-lived `Bitmap` at `ColorType.BGRA_8888` — Skia accepts the
 * RFB wire format directly, so there is NO per-pixel swizzle here (the one genuine difference from
 * the Android actual).
 *
 * ONE native bitmap, (re)allocated only on a framebuffer-size change: a fresh Bitmap per update
 * would churn ~fbW*fbH*4 bytes of wasm-heap memory on every FramebufferUpdate — many/sec on a busy
