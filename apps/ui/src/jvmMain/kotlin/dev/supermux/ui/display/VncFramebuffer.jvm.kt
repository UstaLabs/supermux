...
/**
 * The Skia actual: a flat BGRA byte buffer mutated in place by [VncFrameOps], installed into a
 * long-lived `Bitmap` at `ColorType.BGRA_8888` — Skia accepts the RFB wire format directly, so
 * there is NO per-pixel swizzle here (the one genuine difference from the Android actual).
 *
 * ONE native bitmap, (re)allocated only on a framebuffer-size change: a fresh Bitmap per update
 * would churn ~fbW*fbH*4 bytes of NATIVE (off-heap) memory on every FramebufferUpdate — many/sec on
 * a busy desktop — freed only whenever the JVM Cleaner eventually runs, which -Xmx does not bound.
