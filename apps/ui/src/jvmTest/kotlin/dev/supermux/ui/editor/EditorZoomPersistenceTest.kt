 ...
 *
 *  1. The writeback path (the file pane's `onFontSize` — [UiPrefs.putEditorFontSize]) round-trips
 ...
 *     [EditorSurface]'s `remember(runtimeReady)` safe to
...
class EditorZoomPersistenceTest {
    ...
    fun writeback_persists_and_a_brand_new_engine_init_pushes_the_persisted_size() = runTest {
        ...
        // Simulate the pane's onFontSize: the engine already applied the
