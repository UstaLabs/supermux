...
/// The supermux logo mark, ported from `assets/logo/supermux.svg` (the same source the Compose
/// desktop intro's `SupermuxMark.kt` uses — keep the two in sync) so the first-run intro can
/// stroke-trim and fill the REAL mark live instead of shipping a raster. The SVG is font-glyph
/// output: three contours in a 1015×1015 viewBox, y-flipped through nested group transforms —
/// [paths] bakes that transform stack in, so the returned Paths live in viewBox space and only
/// need a uniform scale to any pixel size.
enum SupermuxMark {
    static let viewBox: CGFloat = 1015

    /// In viewBox space (transforms baked) the mark occupies x[165,865] y[70,945]; its visual
    /// center is within 1% of the box center. Gradient endpoints for stroke/fill shading.
    static let gradientStart = CGPoint(x: 165, y: 70)
    static let gradientEnd = CGPoint(x: 865, y: 945)
    ...
    /// The three bars in viewBox space (group transforms baked). Parsed once.
    static let paths: [Path] = data.map { d in
        let raw = SVGPathParser.parse(d)
        // translate(165 70) → translate(0 875) → scale(0.1 -0.1), applied right-to-left.
        var t = CGAffineTransform(scaleX: 0.1, y: -0.1)
        t = t.concatenating(CGAffineTransform(translationX: 0, y: 875))
        t = t.concatenating(CGAffineTransform(translationX: 165, y: 70))
        return raw.applying(t)
    }
}

/// Minimal SVG path parser covering exactly what the mark's glyph dump uses: M/m, c, l, z with
/// implicit argument repetition (SVG 1.1 §8.3). Numbers are space- OR sign-separated ("l-33 -67").
private enum SVGPathParser {
    static func parse(_ d: String) -> Path {
        ...
        func skipSep() { while i < scalars.count, scalars[i] == " " || scalars[i] == "," { i += 1 } }
        func number() -> CGFloat {
            skipSep()
            let start = i
            if i < scalars.count, scalars[i] == "-" || scalars[i] == "+" { i += 1 }
            while i < scalars.count, scalars[i].isNumber || scalars[i] == "." { i += 1 }
            return CGFloat(Double(String(scalars[start..<i])) ?? 0)
        }
        func hasNumber() -> Bool {
            skipSep()
            guard i < scalars.count else { return false }
            return scalars[i].isNumber || scalars[i] == "-" || scalars[i] == "+" || scalars[i] == "."
        }

        while i < scalars.count {
            skipSep()
            if i < scalars.count, scalars[i].isLetter {
                cmd = scalars[i]
                i += 1
            }
            switch cmd {
            case "M", "m":
                ...
                while hasNumber() {
                    ...
                    if first { path.move(to: p); subStart = p; first = false } else { path.addLine(to: p) }
                    cur = p
                }
                cmd = rel ? "l" : "L" // implicit subsequent pairs are lineTo
            case "c":
                while hasNumber() {
                    let c1 = CGPoint(x: cur.x + number(), y: cur.y + number())
                    let c2 = CGPoint(x: cur.x + number(), y: cur.y + number())
                    let to = CGPoint(x: cur.x + number(), y: cur.y + number())
                    path.addCurve(to: to, control1: c1, control2: c2)
                    cur = to
                }
            case "l":
                while hasNumber() {
                    let p = CGPoint(x: cur.x + number(), y: cur.y + number())
                    path.addLine(to: p)
                    cur = p
                }
            case "z", "Z":
                ...
                cur = subStart
            default:
                i += 1 // not reachable with the mark's data; skip unknown bytes defensively
            }
        }
        return path
    }
