...
enum Theme {
    ...
    /// Terminal surface colors. Defined in `Supermux/Terminal/TerminalTheme.swift` — the iOS
    /// target compiles the terminal host but not this file, so the constants live there and are
    /// re-exposed here for the Mac shell's existing call sites.
    static let terminalBackground = TerminalTheme.background
    static let terminalForeground = TerminalTheme.foreground
}
