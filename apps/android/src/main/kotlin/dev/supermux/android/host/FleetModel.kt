...
/**
 * Pure, framework-free fleet-view model (spec §5). Everything the merged multi-host session list
 * needs that ISN'T Compose: a per-host badge color slot, a compact badge label, an offline
 * "last seen" string, and the host filter. Kept Android-free so it unit-tests on the JVM
 * (mirrors [HostMetaCodec]) and so the UI layer only maps a [colorIndex] → a theme Color.
 */
