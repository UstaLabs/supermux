...
/**
 * Flow-based LSP control-plane + relay — the Android counterpart to iOS `LspBridge.swift`
 * (and the web `stores/lsp.ts`). The broker is a dumb JSON-RPC pipe; the real LSP protocol
 * (initialize, didOpen, completion, hover…) runs inside the WebView's CodeMirror `LSPClient`.
 *
 * Unlike iOS — which keeps per-session continuation waiters fed by inbound `lsp_*` frames —
 * Phase 2 already folds every inbound frame into [AppViewModel] flows ([lspStatus] keyed
 * "session|path", [lspRpc] a SharedFlow). So this bridge just sends the outbound control
 * frames and awaits the corresponding flow transition with the iOS timeouts. It is
 * constructed per editor panel from session-bound lambdas; all RPC is filtered by
 * `session` (the flows are app-wide — never cross-wire another session's server).
 */
class AndroidLspBridge(
     ...
     * LspBridge.swift:36-48). Skips a stale retained value so we wait for a fresh response.
     */
     ...
     * synchronously — see src/core/lsp/bridge.ts:onOpen. On Android these fold into
     * [lspStatus] via `markLspState`. BUT queryStatus already left the matching entries at
     * state="ready" (server *available*), and `lsp_ready` re-applies state="ready" → a
     * value-equal map that StateFlow DEDUPS, so a "ready" flip never emits. A failure,
     * however, flips state to "error"/"exited" → that DOES emit.
     *
     * So: caller has already confirmed status.state=="ready" (installed) and a non-empty
     * workdir (the only two failure modes in onOpen), making success the expected outcome.
     * We therefore treat "no fresh error within a short settle window" as ready, while still
     * catching a real lsp_error/lsp_exit (which emits) and returning false. Identity (===)
     * not value-equality, since lsp_error yields a copy().
     */
     ...
     * delivering each message into the cm6 client via [deliver]. Suspends until cancelled
     * (the caller runs it in a child coroutine of the connect LaunchedEffect, so a tab
     * switch tears it down). Filtering by session + serverId prevents cross-wiring.
     */
