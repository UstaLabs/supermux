...
// JVM actual: P-256 ECDH -> HKDF-SHA256 -> AES-256-GCM via the JCA. This is the
// source-of-truth implementation, exercised by :shared:jvmTest against a real
// broker-sealed vector. The androidMain actual delegates to the same body
// (Android ships the same java.security / javax.crypto APIs).
actual fun openSealedPush(blob: String, privateKeyPkcs8B64: String): String =
    openSealedPushJvm(blob, privateKeyPkcs8B64)

internal fun openSealedPushJvm(blob: String, privateKeyPkcs8B64: String): String {
    ...
    // 5. HKDF-SHA256 (JCA has no built-in HKDF). info = "supermux-push" (ASCII).
